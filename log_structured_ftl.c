...
int main() {
    ...
    unsigned long tmp_processed_size = 0;  // TMP_WAF 계산을 위한 누적 처리된 사이즈
    ...
    while (fscanf(file, "%lf %d %lu %u %u", &request->timestamp, 
        ...
        if (request->io_type == 1) {
            ...
            tmp_processed_size += PAGE_SIZE;

            // 8GiB의 배수가 될 때 WAF와 Utilization을 출력
            if (processed_size >= SSD_SIZE) {

                double WAF = (double)(ssd->gc_write + ssd->user_write) / ssd->user_write;
                double TMP_WAF = (double)(ssd->tmp_gc_write + ssd->tmp_user_write) / ssd->tmp_user_write; // TMP_WAF 계산
                double Utilization = (double)ssd->lba_num / MAX_LBA_NUM;
                int used_block_count = get_used_block_count(ssd); // 사용된 블록의 개수 계산
